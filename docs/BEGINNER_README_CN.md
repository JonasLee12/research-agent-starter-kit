...
5. 如果不知道该走哪条工作流，先看 `docs/TASK_CARDS_CN.md`。
6. 第一个 substantial task 前，复制 `templates/SOURCE_FIRST_INTAKE_CARD.md`。
7. 只填写已经确认的项；不确定的项保留 `TO CONFIRM`。
8. 让 agent 先跑基础检查，不要一上来就写正式正文。
...
| `docs/TASK_CARDS_CN.md` | 帮你在行动前选择最小安全 workflow |
| `templates/SOURCE_FIRST_INTAKE_CARD.md` | 可复制的 intake 表，先明确 source corpus、output surface、evidence boundary 和 gates |
...
- [ ] 创建或自定义 `AGENTS.md`。
- [ ] 如果不知道怎么开始，先读 `docs/TASK_CARDS_CN.md`。
- [ ] 第一个 substantial task 前复制 `templates/SOURCE_FIRST_INTAKE_CARD.md`。
- [ ] 只填写已确认的项目事实。
- [ ] 私人材料不要放进公开 GitHub。
- [ ] 运行基础验证命令。
