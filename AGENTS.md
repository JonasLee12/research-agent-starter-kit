...
34. For Codex `logs_*.sqlite` / WAL growth, diagnose with `scripts/codex_sqlite_log_guard.py scan` or `monitor` first; only apply trigger/checkpoint/archive actions after confirming the target and write-safety boundary.
